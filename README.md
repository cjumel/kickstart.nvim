...
<details>
<summary>From source (preferred)</summary>

This will depend on your system and setup, but what I typically do is:

- download the appropriate Neovim release asset from [the Neovim release
  page](https://github.com/neovim/neovim/releases), for example, for a given release, I choose:
  - `nvim-macos-x86_64.tar.gz` for my personal MacBook, which has an Intel chip
  - `nvim-macos-arm64.tar.gz` for my work MacBook, which has a M4 max chip

- go in the download location and uncompress the asset with the commands recommanded recommanded by
  the release page:

  ```bash
  cd ~/Downloads/
  xattr -c ./nvim-<spec>.tar.gz
  tar xzvf nvim-<spec>.tar.gz
  ```

- move the uncompressed asset to a custom location and create a symbolic link to its executable with
  the following commands:

  ```bash
  mv ~/Downloads/nvim-<spec> ~/.local/nvim-<spec>-<version>
  ln -sf ~/.local/nvim-<spec>-<version>/bin/nvim ~/.local/bin/nvim
  ```
...
# If neovim complains that gcc compiler is missing:
...
# Or, for stable but quite old version:
...
this existing configuration and delete Neovim's temporary files, for instance with the following
commands:
...
Alternatively, you can choose a different directory name, for instance with:

```bash
git clone --depth=1 https://github.com/cjumel/kickstart.nvim ~/.config/nvim-example
```

And then use this configuration thanks to the `APP_NAME` environment variable:

```bash
APP_NAME=nvim-example nvim
```
...
Given that these directories can be deleted at any time, I make sure to avoid saving any important
data in them.
