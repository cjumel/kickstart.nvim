...
local config = {} -- Final, merged configuration
local found_configs = {} -- To keep track of found configurations
...
found_configs["default"] = default_config
...
if global_config_code ~= nil then -- Configuration file is found and trusted
  ...
  found_configs["global"] = global_config
end
...
if project_config_path ~= "" then -- Configuration file is found
  ...
  if project_config_code ~= nil then -- Configuration file is trusted
    ...
    found_configs["project"] = project_config
  end
end
...
if not vim.tbl_isempty(env_config) then
  ...
  found_configs["env"] = env_config
end

vim.g.neovim_config = config
vim.g.found_neovim_configs = found_configs
